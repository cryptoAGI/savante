...
*Re-rendered 2026-09-14 by the operator's remediation pass, not by Savante: the stale facts in findings 1, 2, 6, 7, 10, 12 and 14, risk 7, the rationale and conditions 1, 3, 5 and 7 were re-read from the current tree. The verdict is unchanged. Digests below are **as bound in the ledger at the commit named there** (`savante.commitments.json` `generated_from.repo_head_commit`). No commit sha and no ledger size is written here, because committing and re-binding moves both. `python3 bind/savante_verify.py .` is the live check, and where this document and the ledger disagree, the ledger wins.*
...
- **Integrity** — `savante.commitments.json` (its size moves with every re-bind, so read it with `wc -c savante.commitments.json`; it was 12,718 B when this document was first rendered): per-artifact digests plus a doctrine root over the clauses no owner may edit. This is what a holder actually receives: the ability to prove, without trusting the author, that DEFER is still in the vocabulary.
...
Nine components are ledgered, and beside them the THOT manifest `savante.thot.json` that bundles the eight facets among them. Its identity moves on every re-bind, so it is not copied here: read it from the manifest's `identity` block. Sizes and digests below were read from the generation-2 `savante.commitments.json` `artifacts` on 2026-09-15 and reproduced by `python3 bind/savante_verify.py .`. They are **as bound in the ledger**; none is typed from memory. The `identity` and `facet_agent` rows are the two facets that changed since generation 1 (`savante.thot.json` @ 1fcca89); the ledger that re-binds after they are committed lists nothing under `components_differing_from_head`, and until then its own list is the authority. Every other row is unchanged since generation 1.

**Historical: generation 2. Do not mint from this table.** Every digest below may have moved since (the persona has; generation 7 hashes it to `0xfd1933f2…`). The current values are in `savante.commitments.json` and `PROOF.md`.

| component | path | bytes | sha256 | CIDv1 (raw, sha2-256, base32) |
|---|---|---:|---|---|
| identity | `savante.persona` | 51,117 | `b05357c3be212c116e9b4fd3784d12615984e0b2fba5fa598ddf6a12c3f0df3c` | `bafkreifqknl4hprbfqiw5g2p2n4e2etblgcobmx3ux5ftdo7nijmh4g7hq` |
...
| skill | `.claude/skills/sagi/SKILL.md` | 8,751 | `350da6fcf3cc09eb60516317438f7d3f954a83a8d1f526d9f2e9268a0cf4d6fe` | `bafkreibvbwtpz46mbhvwauldc5by67j7svfihkgr6utnt4xje2faz5gw7y` |
| facet_agent | `sAGI.agent` | 5,461 | `e5a8b0111b3b3d342d5f01057c019044bca46a592ee18ca1ec1e031c4263a650` | `bafkreihfvcybcgz3hu2c2xybav6adecexssguwjo4ggkd3a6amoeey5gka` |
...
| facet_prompt | `sAGI.prompt` | 7,191 | `baf5a302bf591b977d45ff21c484cc1279c9693e0f65ae9541db42c042bdcffe` | `bafkreif26wrqfp2zdolx2rp7ehcijtasphewspqpmwxjkqo3ilaefpop7y` |
| facet_tool | `sAGI.tool` | 3,769 | `ebd3ec9342801ae3dc72840db55e7052b33ff487b6b1fa2e3145e69d9da9ece4` | `bafkreihl2pwjgquadlr5y4uebw2v44cswm77jb5wwh5c4mkf42oz3kpm4q` |
| facet_voaice | `sAGI.voaice` | 1,775 | `d4619d4c6fcf0913ed0e2a1616eb219d2bff9a22efbb323a7e579cf44e25862e` | `bafkreigumgouy36pbej62drkcylowim5fp7zuixpxmzdu7sxtt2e4jmgfy` |
| facet_faice | `sAGI.faice` | 1,866 | `6e2d33f8e6ad06e56c254549020643ab70b55cb61a6cb0328751ba33b7e3991f` | `bafkreidofuz7rzvna3swyjkfjebamq5loc2vznq2nsydfb2rxiz3py4zd4` |

The card: 6,891 B, sha256 `f6d70c0905ecb72794f039038b3d8e677b54fe444058a643961b0a82407873da`, CID `bafkreihw24gasbpmw4tzj4bzaoft3dthpnkp4rcalctehfq3bkbea6dt3i` (v0, `registrations` empty), canonical keccak256 `0xb65236c3fd85125a20580a31636a3e37c0c35d913f7b6016a2f0e385200de3e5` — recorded for an `https://` deployment only. ERC-8004 defines no hash field for `agentURI` at all; the digest is kept by analogy with the EIP's `feedbackHash` / `responseHash` rule (OPTIONAL for IPFS and other content-addressed URIs), not because the EIP asks for it.

For the record, the sizes the implementation spec was written against, at HEAD `c2cccc5dd8a775979045fd580f3549b02f78ee77` (`git show HEAD:<path> | wc -c`): persona 12,675 B, charter 6,955 B, skill 2,880 B. **Superseded on 2026-09-11, and again on 2026-09-14:** the bundle is NINE components, not five. The licence, the four newer facets and the manifest are all committed; `git log -- LICENSE sAGI.prompt sAGI.tool sAGI.voaice sAGI.faice savante.thot.json` names the commits. The earlier statement here that this work was uncommitted, and that these were a working tree's digests, is withdrawn. Whether the digests above are a commit's is not something this document can assert once for all time. It is exactly what `components_differing_from_head` in the ledger says: empty means the ledger hashes the bytes of the commit it names. Non-empty means a working tree, and the ledger's own `note` says so. A ledger that names a commit whose bytes it does not hash is the kind of claim I would flag in anyone else's paperwork.

Files are committed by raw bytes: sha256 plus CIDv1 as constructed by `inft_publish.cid_v1_raw` (inft_publish.py:39-47; docstring :40-43 states the ≤ 256 KiB bound above which an IPFS CID depends on chunking). All nine are far under it. The sha256 is recoverable from the CID by stripping the four-byte prefix `01 55 12 20`. No canonicalization is needed for a file.
...
2. `setMetadata(agentId, "savantePersonaDigest", <onchain_slots.savantePersonaDigest.value>)` and `setMetadata(agentId, "savanteDoctrineRoot", 0x92fe83eb0fb8fb6b9cbde75ee4bbb671032a849ee25d65592b86913d0ae137d0)` — the two 32-byte slots the ledger already holds under `onchain_slots`, each with `written: false`. Read the persona digest from the ledger of the generation being minted, never from this document: it moves with every persona edit (generation 7: `0xfd1933f2dc636a818744edaeade6454e329fddb5438d2ad264e331fd9cc808d3`; the value first written here was generation 2's `0xb05357c3…`, now historical). The doctrine root has not moved since generation 2.
...
*Added 2026-09-14 by the remediation pass:* the offline parameter printer `bind/savante_publish.py` does not target `mintAgent`. It targets `mintOpenAgent(address,bytes32,string,bytes32,uint256,uint8,string)` (savante_publish.py:36-37), the open mint in mindX's `daio/contracts/iNFT2/iNFT_7857.sol` folder. That function takes no `sealedKeyHash` and marks the token `openMint`, and it exists in `iNFT2/` but not in the `inft/` template quoted above. The template is still where the :340 revert lives. The printer signs nothing and sends nothing, and no deployment of either contract is claimed here. Whether `mintOpenAgent` is the operator's answer to the sealedKeyHash question is condition 3, and that condition stays open.
...
Run in this session: twelve KNOWN findings, `VERDICT: APPROVE`, `CONDITIONS: none`, exit 0. (Re-run 2026-09-11 against the nine-component bundle: **nineteen** KNOWN findings, same verdict, same exit code — the four new facets, the derived-prompt check and the manifest reproduction are the additions. Re-run 2026-09-14 against the tree as re-bound: **21** KNOWN findings, `VERDICT: APPROVE`, exit 0. Its first check is now the step-0 `algorithms` check against the closed sagi.thot_manifest/1 vocabulary, which refuses before any digest is computed. The count is a property of the verifier version, and its output is the authority, not this sentence.) Every digest, CID, the doctrine root, the allowlist, the mirror (the generation-2 persona's md5 is `6650816e0b50d05d8e3c12f259459569`; it must be equal at `mindX/mindx/godel/mindxtrain/personas/savante.persona` once the mindX mirror commit lands — the `64182e47…` of 2026-09-14, the `91a98c7c…` of 2026-09-11 and this document's original `ad7310279f…` are stale; the verifier's `mirror md5 … equal` line is the live value) and the v0 card match the ledger. Read that verdict for exactly what it says: it proves the BINDING of these files to this ledger. It does not prove who rendered any verdict, and it compared nothing to a chain. A verifier's APPROVE on the binding and this document's DEFER on the mint are two verdicts on two questions.
...
**Superseded on 2026-09-14 for the copy of these files:** the repository is MIT licensed. `LICENSE` was committed in `0853a34`, its holder line reads `Copyright (c) 2026 cryptoAGI — Professor Codephreak` (set in `29f03bd`), and `gh api repos/cryptoAGI/savante/license --jq .license.spdx_id` prints `MIT`. What a token conveys is still not known: MIT governs the copy of the files, and `token.rights.conveyed_by_token` stays null with its reason. The persona's own `token.rights.license_finding` still carries its 2026-09-12 text ("the file is untracked … OPEN publicly"), which is stale. Correcting it changes the persona bytes, so it needs a re-bind and a fresh copy of the mindX mirror in the same step (condition 5). That edit has not been made. The 2026-09-03 text follows as the record.
...
**Superseded on 2026-09-16 for the naming:** the operator named the artwork, the Savante bust `gfx/Savante3.png`
(154,668 bytes; sha256 `30a59db4ce76dbea8b6a97c143a74996c3c1cd317e3af21f6a18757ae8c5a9a8`; JPEG bytes under a
`.png` name). The instruction is in the Claude Code session named by the `Claude-Session:` trailer of `30d1e8c`.
Generation 7's ledger (`e95a135`) records the file under `image_candidate` with `named_by_operator`, bound with
`--image gfx/Savante3.png --image-named`. The file is under the 256 KiB bound, so its CIDv1 raw is predictable
locally (`bafkreibquwo3jttw3pviw2uxyfb2osmwypa42ml6hlzb62qyov5orrnjva`), but a prediction is not a pin. Nothing is
pinned, so the card's `image` stays null, with the persona's `image_reason` now naming the file. The rest of
`gfx/` is indexed in `gfx/README.md`. The 2026-09-03 text follows as the record.
...
- Whether every teaching row survives `build_corpus` at a stated `persona_share` now that the persona has grown from 12,675 to 51,117 bytes — run it and read the stats before any imprint is claimed.
...
A mint is a visibility-or-publication decision and a treasury action — two of the five defer triggers at savante.persona:136-142 — and the persona's own `token.authority` already records that the signature is the OVERLORD's and Savante's verdict is DEFER (persona:558-563). The binding is sound and reproducible (finding 10), but soundness of the ledger is not authority to publish it: `bind/savante_bind.py` is the operator's tool, Savante verifies the ledger and never writes it, and oversight that mints itself is oversight no longer. The licence is now settled (condition 1, met). The sealedKeyHash question (condition 3) is still a contract decision, and the registry, transfer and ledger-visibility conditions (2, 8, 9) and the pin half of the artwork condition (4; its naming half was met on 2026-09-16) still wait on the operator's signature. None of them can be closed by anything I am permitted to do. DEFER is knowledge about jurisdiction, rendered as precisely as any APPROVE; it is not a soft yes.

## CONDITIONS

Each is independently verifiable by someone who is not me.

1. **License stated.** A `LICENSE` file exists at the repository root, or the terms are stated on github.com/cryptoAGI/savante, and `token.rights.license` is no longer null in a re-bound persona. Check: `ls LICENSE` in a clone, and `gh api repos/cryptoAGI/savante/license --jq .license.spdx_id`. **MET (2026-09-14):** `LICENSE` is tracked, and its holder line reads `Copyright (c) 2026 cryptoAGI — Professor Codephreak`. The gh api check prints `MIT`, and the persona's `token.rights.license` is `"MIT"`.
...
3. **The sealedKeyHash question decided.** The operator chooses between a documented sentinel and a change to `iNFT_7857.sol` so that an unsealed intelligence can be minted without asserting a key; until then `iNFT_7857.sol:340` reverts on zero and no Track 2 mint is attempted. Check: `sed -n '339,340p' /home/hacker/mindX/daio/contracts/inft/iNFT_7857.sol`, and the operator's written decision. Note (2026-09-14): `bind/savante_publish.py` already prints parameters for `mintOpenAgent` in the `iNFT2/` contract folder, which has no sealedKeyHash parameter (finding 7). That is a candidate answer, not a decision, and nothing is deployed or minted. **Open.**
4. **Artwork confirmed.** The operator names the artwork file, or states there is none. If named, its sha256 is recorded under `image_candidate` via `--image <path>` and, once pinned by an IPFS node, the CID that node returns — never a locally guessed one. Check: `image` in the card is either null with a reason or an `ipfs://` URI a gateway resolves to the named bytes. **Naming half MET (2026-09-16):** the operator named `gfx/Savante3.png`; generation 7's ledger (`e95a135`) records its sha256 `30a59db4…c5a9a8` under `image_candidate.named_by_operator`, and the card's `image` is null with a reason that names the file. Check: `sha256sum gfx/Savante3.png` against `image_candidate.sha256` in `savante.commitments.json`. **Pin half open:** no IPFS node has returned a CID for the file, so no `ipfs://` URI is claimed.
5. **Mirror md5 equal.** `md5sum savante.persona /home/hacker/mindX/mindx/godel/mindxtrain/personas/savante.persona` prints one digest twice. For the generation-2 persona: `6650816e0b50d05d8e3c12f259459569`, both, once the mindX mirror commit lands (the verifier's `mirror md5 … equal` line is the live check; the `64182e47…` of 2026-09-14 and the `91a98c7c…` of 2026-09-11 are stale). (When this document was first rendered the value was `ad7310279f756789bf5f162cc2b8133c`; the persona has since gained its licence field, so the digest moved. The CONDITION is that the two inodes agree, not that they hold any particular value.)
...
7. **A commit exists whose bytes the ledger hashes.** `components_differing_from_head` in `savante.commitments.json` is empty. The binder is re-run after the commit; the digests in this document are then superseded by the ledger's and this document is re-rendered. A ledger over an uncommitted tree is a working note, not a record. Status: this document was re-rendered on 2026-09-14 from a working-tree ledger whose `components_differing_from_head` listed `sAGI.agent` (finding 2), so the condition was **not** met for that ledger. It is met for a ledger exactly when that field is empty, so check the field, not this sentence.
...
7. The `token` block grew the persona from 12,675 to 51,117 bytes, and the ERC-7857 `.persona` facet that `inft_publish.py:224` copies verbatim grows with it. Whether every row survives `build_corpus` at a given `persona_share` is not yet known (finding 14).
